...
const PreparationStep: React.FC<PreparationStepProps> = ({
  ...
  // Load preparation phase items
  useEffect(() => {
    const fetchItems = async () => {
      setLoadingItems(true);
      try {
        // Get items specific to preparation phase (standalone items without construct)
        const items = await getConstructItems(null, 'preparation');
        if (Array.isArray(items)) {
          setAvailableItems(items);
        } else {
          console.error('Failed to load preparation items');
          setAvailableItems([]);
        }
      } catch (error) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-semibold mb-4">Přípravná část</h2>
      <p className="mb-6 text-gray-600">
        Vyberte aktivity pro přípravnou část hodiny
      </p>
          ...
          {availableItems.length > 0 ? (
            ...
            <div className="col-span-2 p-6 text-center">
              <p className="text-gray-500">
                Žádné aktivity pro přípravnou část nebyly nalezeny
              </p>
            </div>
          )}
        </div>
